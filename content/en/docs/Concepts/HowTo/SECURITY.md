...
Right now LambdaStack supports only self-signed certificates 
...
To use LambdaStack automation and self-signed certificates you need 
...
In LambdaStack beside storing secrets in Kubernetes secrets there is also a possibility of using secrets stored in Vault
from Hashicorp. This can provide much more sophisticated solution for using secrets and also higher level of security
than standard Kubernetes secrets implementation. Also LambdaStack provides transparent method to access Hashicorp Vault
...
To create user by LambdaStack please provide list of users with name of policy that should be assigned to user. You can
use predefined policy delivered by LambdaStack, default Vault policies or your own policy. Remember that if you have
...
Number of keys will be defined from the level of LambdaStack configuration in the future releases.
...
Values for script configuration in script.config are automatically generated by LambdaStack and can be later on used to
...
If you have configured any user using LambdaStack authentication userpass will be enabled, if not needs to be enabled manually.
...
by LambdaStack, but will be implemented in the future releases.
...
In LambdaStack there is also an option to configure automatically integration with Kubernetes. This is achieved
...
Vault and Kubernetes integration in LambdaStack relies on vault-k8s tool.
...
In LambdaStack you can use integration of key value secrets to inject them into container. To do this you need to create them
using vault CLI.
...
LambdaStack as backend for Vault secrets is using kv secrets engine. More information about kv secrets engine you can find
[here](https://www.vaultproject.io/docs/secrets/kv).

### Kubernetes namespace

In LambdaStack we are creating additional Kubernetes objects to inject secrets automatically using sidecar. Those objects
